...
test('item 7: diagnose reports dynamic activation facts without stale pending text', async (t) => {
  ...
  await writeFile(join(env.CLAUDE_CONFIG_DIR, 'plugins', 'installed_plugins.json'), JSON.stringify({ plugins: { 'fabex@fabex': [{ version: '1.8.1', installPath: pluginRoot }] } }));
  ...
  assert.equal(diagnosed.activation.sourceVersion, '1.8.1'); assert.equal(diagnosed.activation.hooksValid, true); assert.match(diagnosed.activation.verdict, /not verified/); assert.doesNotMatch(result.stdout, /pending plugin install\/reload and live dogfood/);
  const current = await readState(project, env); await updateState(project, (state) => { state.partner.thread.metadata.lastRecordedTurn = { at: new Date().toISOString(), version: '1.8.1' }; state.generation += 1; return state; }, { expectedGeneration: current.state.generation }, env);
...
test('item 9: nested repository status separates live and captured fingerprints and refreshes on success', async (t) => {
  ...
  assert.match(state.partner.thread.checkpoint.updatedAt, /^\d{4}-/); assert.equal(state.partner.thread.metadata.lastRecordedTurn.version, '1.8.1'); assert.deepEqual(state.partner.thread.checkpoint.repoFingerprint, state.partner.thread.metadata.repoFingerprint); assert.ok(state.partner.thread.metadata.repoFingerprintCapturedAt);
...
test('item 10: current metadata and 1.5.1 migration documentation stay consistent', async () => {
  assert.equal(JSON.parse(await readFile(join(pluginRoot, 'package.json'), 'utf8')).version, '1.8.1');
  assert.equal(JSON.parse(await readFile(join(pluginRoot, '.claude-plugin', 'plugin.json'), 'utf8')).version, '1.8.1');
