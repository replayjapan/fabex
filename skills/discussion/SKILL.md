...
Use the jointly 1.7.1 protocol for approved screenshot paths in `attachments` and structured reviews. Both image phases keep the SDK's read-only sandbox and independent-first sequencing. Paste the complete result `relayBlock` for each phase before Fable's view, with Phase 2 corrections separately visible. Never replace Codex's full words with a merged summary. Owner-requested interruption may waive a completed cycle through `recover abandon`.
