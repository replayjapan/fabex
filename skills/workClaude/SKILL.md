...
This skill is owner-invoked. Use its `UserPromptExpansion` grant exactly once with `node ${CLAUDE_PLUGIN_ROOT}/scripts/control.mjs mode normal --participants claude --grant <provided-id>`. If no grant is present, do not change mode. Show the mode message and configured badge.
