...
export function renderSessionContext(route, participants, config) {
  ...
  if (route === 'normal' && participants === 'both') {
    ...
    return `Fabex mode: ${label}. ${badge} Joint default ${joint}; /fabex:jointly. Both means every owner message consults the continuous primary Codex thread, including greetings and lookups. Questions authorize answers only; never infer implementation. For ANY build, fix, change, or implement request you MUST invoke /fabex:jointly first. Codex alone edits files. Current-turn opinion-blind: ask Codex before publishing Claude's complete current view; fetch only afterward. Share owner-visible history, prior decisions, and the owner message verbatim. Verify every resumed thread ID; mismatch pauses for the owner. Repo observations may be stale. Delegate image/screenshot, GitHub/gh, and log chores to fabex-operational.`;
  }
