...
export function renderSessionContext(route, participants, config) {
  ...
  if (route === 'normal' && participants === 'both') {
    ...
    return `Fabex mode: ${label}. ${badge} Joint default ${joint}; /fabex:jointly. Both means every owner message is queued on one canonical Codex SDK thread across mode changes and restarts. Questions authorize answers only. Codex alone edits files; Claude main-session Write/Edit/NotebookEdit is denied unless an owner-named executor exception is recorded. Submit through the Fabex controller, report genuine lifecycle status, verify the exact thread.started ID, and relay scope/parity flags unedited. Delegate image/screenshot, GitHub/gh, and log chores to fabex-operational with the configured model explicitly.`;
  }
  ...
  if (route === 'discussion' && participants === 'both') return `Fabex mode: ${label}. ${badge} Persistent joint discussion. Queue every owner message on the canonical Codex SDK thread with a per-turn read-only sandbox, then converge. /work exits.`;
  ...
  if (route === 'ask-once' && participants === 'both') return `Fabex mode: ${label}. ${badge} One-shot joint read-only answer on the canonical SDK thread. The next prompt restores the prior mode.`;
