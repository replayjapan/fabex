...
export function renderSessionContext(route, config) {
  if (route === 'normal') {
    ...
    return `Fabex route: normal. Joint default ${joint}; /fabex:jointly. Questions authorize answers only; never infer implementation. For ANY build, fix, change, or implement request you MUST invoke /fabex:jointly first. Codex alone edits files; never use Write/Edit, shell, or node scripts, and never ask the user for write access. Design/judgment questions: jointly gets blind independent views, then converges. Skip greetings/trivial lookups; /askClaude or /askCodex = one AI. You MUST delegate every image or screenshot inspection, GitHub or gh command sequence, and log-dump analysis to fabex-operational; do not perform any part of those in the primary session. /discussion is persistent read-only.`;
  }
