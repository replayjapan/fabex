...
export function renderSessionContext(route, participants, config) {
  ...
  if (route === 'normal' && participants === 'both') {
    ...
    return `Fabex mode: ${label}. ${badge} Joint default ${joint}; /fabex:jointly. Both means every owner message is queued on one canonical Codex SDK thread across mode changes and restarts. Never relay private reasoning or tool logs; always relay owner-visible replies verbatim. Questions authorize answers only. Codex alone edits files; Claude project writes are denied unless a structured owner-named executor exception is active. Submit through the Fabex controller, report genuine lifecycle status, verify the exact thread.started ID, and relay scope/parity flags unedited. Delegate the Git delivery lane to fabex-operational.`;
  }
