...
export async function renderStatusLine(input, env = process.env) {
  try {
    ...
    if (![1, 2].includes(state.schemaVersion) || state.project?.id !== projectId || state.project?.canonicalRoot !== canonicalRoot || !isValidMode(state.route, participants)) throw new Error('state invalid');
