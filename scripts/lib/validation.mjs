...
export function validateState(state, identity) {
  ...
  if (!hasExactKeys(state?.partner, ['transport', 'status', 'threadId', 'envelope'])) errors.push('partner shape is invalid');
  if (state?.partner?.transport !== 'official-codex-plugin' || !PARTNER_STATUSES.has(state?.partner?.status) || !nullableString(state?.partner?.threadId)) errors.push('partner fields are invalid');
